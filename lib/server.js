  ...
  var Server = classes.Base.$define('Server', {
    ...
    sendToId       : function (id, type, array){
      ...
      if (type && _.isFunction(this.$class.commands[type])) {
        if (id && _.has(this.clients, id)) {
          this.$class.commands[type].apply(this.clients[id], array).done(
    ...
    getStratumCommands: function (buffer){
      ...
      if (Buffer.isBuffer(buffer)) {
        ...
        var _tmp = string.split('\n');
        if (_tmp.length) {
          cmds = _.filter(_tmp, function serverCommandsFilter(item){ return !_.isEmpty(item) && !_.isNull(item); });
        }
      }
    ...
    invalidArgs: function(id, name, expected, args) {
      args = _.filter(args, function(i){ return typeof i !== 'undefined'; }).length;
      if (id === null || args !== expected) {
          return classes.Server.rejected(
            !id ? 'No ID provided' : 'Wrong number of arguments in "' + name + '", expected ' + expected +
            ' but got ' + args
          );
      }
      return true;
    ...
    commands          : {
      ...
      update_block: function(id, result) {
        var ret;
        if ((ret = classes.Server.invalidArgs(id, 'update_block', 1, arguments)) !== true){
          return ret;
        }
      ...
      notify          : function (id, job_id, previous_hash, coinbase1, coinbase2, branches, block_version, nbit, ntime, clean){
        var ret;
        if ((ret = classes.Server.invalidArgs(id, 'notify', 9, arguments)) !== true){
          return ret;
        }
