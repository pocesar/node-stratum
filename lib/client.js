...
class Client extends base_1.Base {
    ...
    fullfill(command) {
        ...
        if (_.has(command, "error") &&
            ...
            if (_.has(command, "method")) {
                ...
                if (server_1.Server.commands[method[1]].broadcast === true ||
                    method[1] === "error") {
    ...
    handleData(socket, buffer) {
        var c = server_1.Server.getStratumCommands(buffer), cmds = c.cmds;
        server_1.Server.processCommands.call(this, this, cmds);
    }
    /**
     ...
     */
    set_difficulty(args) {
        return server_1.Server.commands.set_difficulty.apply(this, [null].concat(args));
    }
    /**
     * Don't use this functions directly, they are called from the server side
     * it's not a client side command, but an answer
     ...
     */
    notify(args) {
        return server_1.Server.commands.notify.apply(this, [null].concat(args));
    }
    /**
     * Send HTTP header
     *
    ...
    stratumSend(data, bypass, name) {
        if (this.authorized === true || bypass === true) {
            ...
            var error = new Error(Client.debug(server_1.Server.errors.UNAUTHORIZED_WORKER));
            this.emit("mining.error", error);
            return server_1.Server.rejected(error);
        }
    }
    /**
