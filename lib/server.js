  ...
  var Server = classes.base.define('Server', function StratumServer(){
    return {
      construct      : function (opts){
        ...
        self.rpc.expose('mining.connections', Server.expose(self, 'mining.connections'), self);
        self.rpc.expose('mining.update_block', Server.expose(self, 'mining.update_block'), self);

        self.server = classes.net.createServer();

        Server.debug('Created server');

        self.server.on('connection', self.newConnection.bind(self));
      },
      /**
       * @param {Client} socket
       */
      ...
      handleData     : function (socket, buffer){
        ...
        if (/ HTTP\/1\.1\n/i.test(string)) {
          ...
          ).done(this.closeConnection.bind(this, socket));
        } else if (cmds.length) {
        ...
        try {
          ...
          if (
            ...
            if (method.length === 2 && typeof classes.server.commands[method[1]] === 'function') {
              ...
              // We don't want socket's messing around with broadcast functions!
    ...
    bindCommand           : function (socket, type, id){
      return function(){
        return Server.commands[type].apply(socket, [id].concat(_.toArray(arguments)));
      };
    },
    rejected: function(msg){
      var d = q.defer();

      d.reject(Server.debug(msg));
    ...
    defaults          : {
      ...
      coinds  : {
        'bitcoin' : {
          ...
          path    : '/usr/sbin/bitcoind',  // path to the coind daemon to spawn
        ...
        'litecoin': {
          enable  : false,                 // enable this coind
          path    : '/usr/sbin/litecoind',  // path to the coind daemon to spawn
          user    : 'user',                // RPC username
          password: 'password',            // RPC password
          port    : 9332,                  // RPC port
          host    : '127.0.0.1',           // RPC host
        ...
        'ppcoin'  : {
          enable  : false,                 // enable this coind
          path    : '/usr/sbin/ppcoind',    // path to the coind daemon to spawn
          user    : 'user',                // RPC username
          password: 'password',            // RPC password
          port    : 9902,                  // RPC port
          host    : '127.0.0.1',           // RPC host
