...
export class Client extends Base {
    ...
    if (
      ...
      if (_.has(command, "method")) {
        ...
        if (
          ...
          throw new Error(
            "Server sent unknown command: " + JSON.stringify(command)
          );
        }
      } else {
        throw new Error(
          "Broadcast without a method: " + JSON.stringify(command)
        );
      }
    } else if (_.has(command, "id") && _.has(self.pending, command["id"])) {
      // need to resolve pending requests by id
      Client.debug(
        "Received pending request response: " + command + " " + self.pending
      );
  ...
  handleData(socket, buffer) {
    var c = Server.getStratumCommands(buffer),
      cmds = c.cmds;

    Server.processCommands.call(this, this, cmds);
  }
   ...
   */
  set_difficulty(args) {
    return Server.commands.set_difficulty.apply(this, [null].concat(args));
  }
   ...
   */
  notify(args) {
    return Server.commands.notify.apply(this, [null].concat(args));
  }
