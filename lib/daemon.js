...
class Daemon extends base_1.Base {
    constructor(opts) {
        super();
        ...
        if (!_.isPlainObject(opts)) {
            ...
            if (!_.every(mustHave, function (item) {
                ...
                throw new Error('Daemon must have options "' +
                    mustHave.join(", ") +
        ...
        if (!_.has(opts, "path")) {
            ...
            if (!_.has(opts, "datadir")) {
                throw new Error('The option "datadir" must be set to the place your wallet.dat is set.');
            }
        ...
        this.rpc = new json_rpc2_1.Client(opts.port, opts.host, opts.user, opts.password);
        this.process = null;
    }
    /**
    ...
    close(wait) {
        ...
        return new q(function (resolve, reject) {
            if (!_.isNull(self.process)) {
                ...
                self
                    .call("stop")
                    .then(function daemonProcessStopSuccess() {
    ...
    call(method, params) {
        ...
        return new q(function (resolve, reject) {
            ...
            self.rpc.call(method, _.isArray(params) ? params : [params], function daemonRPCCall(err, result) {
                clearTimeout(timeout);
                if (err) {
                    return reject(err);
                }
