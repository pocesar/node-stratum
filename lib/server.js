...
class Server extends base_1.Base {
    constructor(opts) {
        ...
        if (opts.rpc) {
            self.rpc = new rpc_1.RPC(self.opts.rpc);
    ...
    addDaemon(definition) {
        ...
        this.daemons[definition.name] = new daemon_1.Daemon(definition);
        return this;
    }
    /**
    ...
    newConnection(socket) {
        ...
        if (this._tooBusy()) {
            ...
            socket = new client_1.Client(socket, true);
    ...
    static processCommands(socket, cmds) {
        var command, method, self = this, onClient = this instanceof client_1.Client, onServer = !onClient && this instanceof Server;
    ...
    static expose(base, name) {
        return function serverExposedFunction(args, connection, callback) {
            return new q(function (resolve, reject) {
                rpc_1.RPC.debug('Method "' + name + '": ' + args);
                ...
                res = [].concat(res);
                rpc_1.RPC.debug('Resolve "' + name + '": ' + res);
                callback.call(base, null, [res[0]]);
            }, function serverExposedReject(err) {
                rpc_1.RPC.debug('Reject "' + name + '": ' + err);
