          ...
          if (
            ...
            if (method.length === 2 && typeof classes.Server.commands[method[1]] === 'function') {
              ...
              if (!(method[1] in ['set_difficulty', 'notify', 'error'])) {
                ...
                self.emit('mining', method[1], d, socket);
              } else {
            ...
            } else if (command['id'] == socket.currentId) {
              console.log('Received command ' + command);
              self.setLastCommand(command);
            } else {
              self.emit('mining.error', self.$class.debug('Mismatched call id, was expecting ' + self.currentId + ' but got ' + command['id']), socket);
            }
          } else {
