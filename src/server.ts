...
export class Server extends Base {
  ...
  closeConnection(socket) {
    ...
    Server.debug(
      "(" + id + ") Closed connection " + _.size(this.clients) + " connections"
    );
  }
  ...
  addDaemon(definition) {
    ...
    this.daemons[definition.name] = new Daemon(definition);
  ...
  static processCommands(socket, cmds) {
    var command,
      method,
      self: Server | Client = this as any,
      onClient = this instanceof Client,
      onServer = !onClient && this instanceof Server;
      ...
      try {
        ...
        if (
          ...
          if (
            ...
            if (
              Server.commands[method[1]].broadcast !== true &&
              method[1] !== "error"
            ) {
            ...
            Server.commands.error.call(
              socket,
              command.id,
              Server.errors.METHOD_NOT_FOUND
            );

            throw new Error('Method not found "' + command.method + '"');
          }
        } else if (
          (onClient || socket.byServer === true) &&
  ...
  static commands = {
    ...
    authorize(id?, authorized?) {
      ...
      if (!this.subscription) {
        return Server.commands.error.call(
          this,
          id,
          Server.errors.NOT_SUBSCRIBED
        );
      }
    ...
    get_transactions(id?, merkles?) {
      var ret;
      if (
        (ret = Server.invalidArgs(id, "get_transactions", 1, arguments)) !==
        true
      ) {
        return ret;
      }
    ...
    set_difficulty(id?, value?) {
      var ret;
      if (
        (ret = Server.invalidArgs(false, "set_difficulty", 1, arguments)) !==
        true
      ) {
        return ret;
      }
